...
async def customize_night(client, message: Message, _):
    Yukinon = await message.reply(_["nm2"])
    if message.chat.type == "private":
        return await Yukinon.edit(_["nm5"])
    if message.chat.type == "channel":
        return
    else:
        parameter = message.text.split(None, 1)[1]
        if len(message.command) < 3:
            return await Yukinon.edit(_["nm11"])
        if "|" in parameter:
            zone, ctime, otime = parameter.split("|")
        else:
            return await Yukinon.edit(_["nm12"])
        zone = zone.strip()
        ctime = ctime.strip()
        otime = otime.strip()

        if len(ctime) != 11:
            return await Yukinon.edit(_["nm13"])

        if len(otime) != 11:
            return await Yukinon.edit(_["nm13"])

        if not zone and ctime and otime:
            return await Yukinon.edit(_["nm14"])
        ...
        if ttime is None or otime is None or ctime is None:
            return await Yukinon.edit("Please enter valid `date`, `time` & `zone`")
        ...
        if cctime == ootime:
            return await Yukinon.edit(_["nm15"])

        if not ootime > cctime and not cctime < ootime:
            return await Yukinon.edit(_["nm16"])
        ...
        await Yukinon.edit(f"**Nightmode set successfully in {message.chat.title} chat !**")
