...
async def is_using_Yukinon(user_id):
...
async def add_user(user_id):
    is_using = await is_using_Yukinon(user_id)
    if is_using:
        return
    return await usersdb.insert_one({"user_id": user_id}) 
...
async def is_Yukinon_in_groups(group_id):
...
async def add_group(group_id):
    is_using = await is_Yukinon_in_groups(group_id)
    if is_using:
        return
